...
(cd collector && python -m pytest -q tests/test_live_integration.py)
```
...
ks-module match --page ./journey-output/page-evidence.json   --rules ./collector/examples/demo-rules.json   --store ./journey-output/demo-store --output ./journey-output/matches.json
...
For reference validation, use `(cd canonical && python validate.py)`. It validates the included bundle; it does not accept a collector bundle or run the missing canonical service.

For the protocol, use `(cd protocol && python validate_protocol.py)`. Only the shipped example operation/signature is registered. There is no working `ks-program`, `ks-enroll`, `ks-send`, generic `--module KN-02`, or HTTP service command in this package.
